...
import { formatCents, minYYYYMMDD, todayYYYYMMDD } from "../utils/general"
...
export default function AnalyticsPage() {
  ...
  const [filterAnalyticsData, setFilterAnalyticsData] =
    useState<FilterAnalyticsForm>({
      from: "",
      to: "",
    })
  ...
  function handleFilterAccountDataChange(
    field: FilterAnalyticsField,
    value: string
  ) {
  ...
  function getPastDaysRange(daysBack: number) {
    const to = new Date()
    const from = new Date()
  ...
  function getPastMonthsRange(monthsBack: number) {
    const to = new Date()
    const from = new Date()
  ...
  function applyQuickRange(next: { from: string; to: string }) {
    setDraftFrom(next.from)
    setDraftTo(next.to)
    setFilterAnalyticsData(next)
  }

  const { data: analyticsData } = useAnalyticsQuery({
    ...filterAnalyticsData,
    account_ids: recentAccount?.id ?? "",
  })
  ...
  const insightsQuery = useQuery({
    queryKey: [
      "insights",
      recentAccount?.id ?? "",
      filterAnalyticsData.from,
      filterAnalyticsData.to,
    ],
    queryFn: () =>
      getInsights({
        account_ids: recentAccount!.id,
        from: filterAnalyticsData.from,
        to: filterAnalyticsData.to,
      }),
    placeholderData: (prev) => prev,
    enabled: !!recentAccount?.id,
    retry: 1,
  })
      ...
      <div className="mx-auto max-w-7xl p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Check the summary for your accounts and understand your financial
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => applyQuickRange(getPastDaysRange(7))}
            ...
            <button
              type="button"
              onClick={() => applyQuickRange(getPastMonthsRange(1))}
              className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
            >
              Past month
            </button>
            ...
            <button
              type="button"
              onClick={() => applyQuickRange(getPastMonthsRange(6))}
              className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
            >
              Past 6 months
            </button>
            ...
            <button
              type="button"
              onClick={() => applyQuickRange(getPastMonthsRange(12))}
              className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
            >
              Past year
            </button>
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Generate a user-friendly summary based on your current analytics
                filters.
              </p>
            </div>
            ...
            <button
              ...
              disabled={insightsQuery.isFetching || !recentAccount?.id}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-sm text-indigo-700">
                    Looking at your filtered activity, categories, and spending
                    patterns.
                  </div>
                </div>
              </div>
          ...
          !insightsQuery.isError ? (
            <div className="mt-6 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
              No AI insights yet. Click{" "}
              <span className="font-medium text-slate-700">
                Generate insights
              </span>{" "}
              to analyze the current filtered transactions.
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {(analyticsData?.by_category ?? []).length === 0 ? (
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-8 text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Try adjusting your filters or add more transactions to see
                analytics here.
              </p>
            </div>
          ) : null}
